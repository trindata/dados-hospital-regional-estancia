...
const CONFIG = {
  ...
  RANGE_PACIENTES: `A${PRIMEIRA}:Q${ULTIMA}`, // Dados dos pacientes
  ...
  COL_PROFISSIONAL: 16, // P: Profissional responsável (coluna de DADOS)
  COL_AVALIACAO: 17, // Q: Avaliação Diária
...
const VALIDACOES_COLUNAS = {
  ...
  [CONFIG.COL_VNI]: { lista: CONFIG.VALIDACOES.VNI },
  [CONFIG.COL_DESMAME_VM]: { lista: CONFIG.VALIDACOES.DESMAME_VM },
  ...
  // Dropdown vivo: alimentado pelos fisioterapeutas escalados no turno
...
const FORMATO_COLUNAS_DADOS = {
  ...
  // Largura apenas (sem alinhamento/wrap): antes vinha de _ajustarColunas,
  // removida do núcleo por enumerar colunas nome a nome.
  [CONFIG.COL_DESMAME_VM]: {
    horizontalAlignment: "center",
    width: CONFIG.LARGURAS.DESMAME_VM,
  },
};
